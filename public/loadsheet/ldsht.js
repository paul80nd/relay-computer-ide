// Render a load-sheet view of an assembled program.
//
// The host IDE writes the payload to localStorage under `rcide_export_<key>` and opens
// (or hash-updates) this page with `#k=<key>`. We read the payload here, remove the
// storage entry, and render. Re-renders on `hashchange` so the host can update an
// already-open popup by writing a new key and changing our hash.
//
// Payload format: `<prgid>@<name>@<desc>@<dasm>` (dasm with '\n' replaced by '|').

function renderLoadSheet(payload) {
...
function readAndRender() {
  ...
  if (!m) return;
  const storageKey = 'rcide_export_' + m[1];
  const payload = localStorage.getItem(storageKey);
  if (payload === null) return;
  localStorage.removeItem(storageKey);
  document.body.innerHTML = renderLoadSheet(payload);
}
