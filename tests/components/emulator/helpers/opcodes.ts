...
/** ALU_TO_A: fff  (f = func index per aluFunc) */
export const ALU_TO_A = (f: number) => 0x80 | (f & 0x7);
/** ALU_TO_D: fff  (f = func index per aluFunc) */
export const ALU_TO_D = (f: number) => 0x88 | (f & 0x7);
/** LOAD: dd */
export const LOAD_TO = (d: number) => 0x90 | (d & 0x3);
/** STORE: ss */
export const STORE_FROM = (s: number) => 0x98 | (s & 0x3);
/** MOV16: d ss (d=0 -> XY, d=1 -> PC; s: 0=M,1=XY,2=J,3=0) */
export const MOV16 = (d: number, s: number) => 0xa0 | ((d & 0x1) << 2) | (s & 0x3);
/** LDSW: d (d=1 -> D, d=0 -> A) */
export const LDSW_TO = (toD: boolean) => 0xac | (toD ? 0x01 : 0x00);
/** HALT: r (r=1 -> jump to PS, else stop) */
export const HALT = (r: 0 | 1 = 0) => 0xae | (r & 0x01);
/** INCXY */
