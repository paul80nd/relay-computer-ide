...
export function onMonacoLoad() {
  ...
  monaco.languages.registerSignatureHelpProvider('rcasm', {
   ...
   signatures: {
    ...
    provideSignatureHelp(model, position, token): monaco.languages.ProviderResult<monaco.languages.SignatureHelpResult> {
     ...
     const match = textUntilPosition.match(/^([a-z]{3}) (?:([a-z0-9]*)(,([a-z0-9]*))?)?$/i);
     if (match && match[1]) {
       // Get signature
       const mnemonic = match[1].toLowerCase();
       const signatures = this.signatures[mnemonic];
       const atParam = match[3] ? 1 : 0;
       // Filter signatures
       return {
         value: {
  ...
  monaco.languages.registerHoverProvider('rcasm', {
    provideHover(model, position, token): monaco.languages.ProviderResult<monaco.languages.Hover> {
      ...
      const match = line.match(/^([a-z]{3})(?: ([a-d])(?:,([a-d]|[01]+b|0x[0-9a-f]+|-?[0-9]+))?)?/i);
      if (match) {
        if (match[1]) {
